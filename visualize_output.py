...
img = cv2.imread(os.path.join('Coco_Dataset', 'train2017', '000000000241.jpg'))
...
conf_map = np.zeros((92, 92))
for i in range(conf.shape[2]):
    conf_map += conf[:, :, i]

conf_map = cv2.resize(conf_map, (368, 368))

#conf_map = (conf_map > 0.7).astype(np.float32)

cv2.imshow('conf map', conf_map*25500000)
cv2.waitKey()
cv2.destroyAllWindows()

# Visualize Parts Affinity Fields
paf_map = np.zeros((92, 92))
for i in range(paf.shape[3]):
    paf_map += paf[:, :, 0, i] + paf[:, :, 1, i]

paf_map = cv2.resize(paf_map, (368, 368))
